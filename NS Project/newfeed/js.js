...
function fetchNews(category) {
    fetch(`https://content.newtonschool.co/v1/pr/64e3d1b73321338e9f18e1a1/inshortsnews?category=${category}`)
        .then(response => response.json())
        .then(data => {
            displayNews(data);
        })
    ...
    news.forEach(item => {
        const newsCard = `
            <div class= "news-card " >
                ...
                <button onclick="saveNews('${item.author}','${item.category}','${item.content}','${item.url}', this)" id="btn2" >Save</button>
            </div>
        `;
        ...
        const savedNewsCard = `
            <div class="news-card">
                ...
                <div class="abhi">
                    ...
                    <button onclick="deleteNews(${index})" id = "btn1">Delete</button>
                </div>
            </div>
        `
